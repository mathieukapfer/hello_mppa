...
int main(){
  cl_int err;
...
#else
    const char filename[] ="opencl-sample4.cl";
    char *program_buffer;
    size_t program_size;
    ...
    const char filename[] ="output/opencl_kernels/opencl-sample4.cl.pocl";
    ...
    cl::CommandQueue queue_cmd(context, default_device);
    //create queue to which we will push commands for the device.
    cl::CommandQueue queue_data(context, default_device);

    //write arrays A and B to the device
    queue_data.enqueueWriteBuffer(buffer_A,CL_TRUE,0,sizeof(int)*10,A);
    queue_data.enqueueWriteBuffer(buffer_B,CL_TRUE,0,sizeof(int)*10,B);
    queue_data.enqueueWriteBuffer(buffer_magic,CL_TRUE,0,sizeof(int),&magic);
    ...
    queue_cmd.enqueueNDRangeKernel(kernel_add,cl::NullRange,cl::NDRange(10),cl::NullRange);

    for (int i=0; i<5; i++) {
      sleep(1);
      printf("enqueueWriteBuffer %d\n", i);
      queue_data.enqueueWriteBuffer(buffer_magic, CL_FALSE, 0, sizeof(int),&i);
    }
    ...
    queue_data.enqueueReadBuffer(buffer_C,CL_TRUE, 0, sizeof(int)*10, C);
